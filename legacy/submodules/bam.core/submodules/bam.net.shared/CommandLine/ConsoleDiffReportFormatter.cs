...
namespace Bam.Net.CommandLine
{
    public class ConsoleDiffReportFormatter: DiffReportFormatter
    {
        ...
        {
            Message.PrintLine("{0}{1}", ConsoleColor.Cyan, NumberLines ? "{0} "._Format(lineNumber) : "", text);
        }

        public override void WriteDeletedLine(int lineNumber, string text, StringBuilder output)
        {
            Message.PrintLine("-{0}{1}", ConsoleColor.Red, NumberLines ? "{0} "._Format(lineNumber) : "", text);
        }

        public override void WriteInsertedLine(int lineNumber, string text, StringBuilder output)
        {
            Message.PrintLine("+{0}{1}", ConsoleColor.Green, NumberLines ? "{0} "._Format(lineNumber) : "", text);
        }
    }
}
