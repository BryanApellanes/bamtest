...
namespace BamTest
{
    public class TestAssemblyRunner
    {
        ...
        public TestProjectResult Run(string assemblyPath, string testSwitch, CoverageOptions? coverage = null)
        {
            ...
            if (coverage != null)
            {
                string coverageFile = $"{assemblyName}.coverage.{CoverageOptions.GetExtensionForFormat(coverage.Format)}";
                ...
                arguments = $"collect --output \"{coverageFile}\" --output-format {coverage.Format}{settingsArg} -- dotnet \"{assemblyPath}\" --{testSwitch}";
