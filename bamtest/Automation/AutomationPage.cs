...
namespace Bam.Net.Testing.Automation
{
    public class AutomationPage : IAutomationPage, IPage, IDisposable
    {
        ...
        public string ScreenShotsDirectory
        {
            get
            {
                if (string.IsNullOrEmpty(screenshotsDirectory))
                {
                    return new FileInfo("./bamtest_screenshots").FullName;
                }
        ...
        protected async Task<FileInfo> TakeDebugScreenShotAsync(string imageName)
        {
            if(Debug)
            {
                string path = Path.Combine(ScreenShotsDirectory, "debug", imageName);
